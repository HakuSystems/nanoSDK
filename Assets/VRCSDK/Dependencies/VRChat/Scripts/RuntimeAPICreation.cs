...
namespace VRCSDK2
{
    ...
    public class RuntimeAPICreation : MonoBehaviour
    {
        ...
        {
            if (UnityEditor.EditorUtility.DisplayDialog("Modded SDK", "Update Complete! Launch VRChat to see your uploaded content." + (null==contentUrl ? "" : "\n\nManage content at: " + contentUrl ), (null == contentUrl) ? "Okay" : CommunityLabsConstants.MANAGE_WORLD_IN_BROWSER_STRING, (null == contentUrl) ? "" : "Done" ))
            {
                if (null!=contentUrl)
                {
        ...
        protected void OnSDKPipelineError(string error, string details)
        {
            ...
            UnityEditor.EditorUtility.ClearProgressBar();
            if (cancelRequested)
                UnityEditor.EditorUtility.DisplayDialog("Modded SDK", "The update was cancelled.", "Okay");
            else
                UnityEditor.EditorUtility.DisplayDialog("Modded SDK", "Error updating content. " + error + "\n" + details, "Okay");
        }
